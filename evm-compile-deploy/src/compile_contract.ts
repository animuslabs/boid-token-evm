...
export async function compileContract(
  ...
  // Load Solidity version 0.8.23
  const compilerVersion = "v0.8.23+commit.f704f362";
  const solcInstance = await new Promise<solc.Compiler>((resolve, reject) => {
    solc.loadRemoteVersion(compilerVersion, (err: Error | null, solcInstance: any) => {
      if (err) {
        reject(`Error loading Solidity compiler: ${err}`);
      } else {
  ...
  console.log(`Using Solidity compiler version: ${compilerVersion}`);
  ...
  const input = {
    ...
    settings: {
      ...
      outputSelection: {
        "*": {
          "*": ["abi", "evm.bytecode.object", "metadata", "storageLayout"],
        },
      },
    },
  };
  ...
  const storageLayoutPath = path.resolve(artifactsDir, `${contractName}_storage.json`);
  fs.writeFileSync(
    storageLayoutPath,
    JSON.stringify(contractOutput.storageLayout, null, 2),
    "utf8"
  );
